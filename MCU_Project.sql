...
# MCU's revenue per year (SKILLS: Aggregate Function)
SELECT
...
# Revenue by movie as a percentage of total franchise revenue (SKILLS: Aggregate Window Function)
SELECT movie,
...
# Domestic box office compared to total box office per movie (SKILLS: Operator)
SELECT movie, ROUND((domestic_box_office/worldwide_box_office) * 100, 2) AS percent_domestic
FROM box_office
ORDER BY percent_domestic;

# Gross profit & margin per movie (SKILLS: CTE, Join, Operator)
WITH gross AS (
	SELECT movie,
			release_date,
            production_budget,
...
# Highest-grossing MCU movie each year (SKILLS: CTE, Aggregate Function, Join)
...
# Highest-rated MCU movie each year (SKILLS: CTE, Window Function)
WITH rankings AS (
	SELECT EXTRACT(year FROM release_date) AS year,
			movie,
            rating,
...
# Comparing number of leading roles to number of MCU appearances for each actor appearing in > 1 movie (SKILLS: CTE, Aggregate Function, Join)
